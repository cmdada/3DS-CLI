 ...
 *
 * Nothing on this console points at the panel, so the grid is walked with the
 * d-pad and committed with cross - the same path the GameCube uses. A real
 * USB keyboard, where one is plugged in, bypasses this entirely and pushes
 * into the ring from plat_poll_keyboard.
 */
...
void pkbd_draw(void) {
  ...
  /* The display scans X8R8G8B8 and base is already offset past the X; see
     PLAT_PIXEL_RGB in plat_cfg.h. */
